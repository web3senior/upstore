                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="<?= URL ?>panel/<?= $this->endpoint ?>/insert" method="post" enctype="multipart/form-data" autocomplete="off">
                            <div>
                                <fluent-text-field appearance="filled" name="tile_url">نقشه پایه</fluent-text-field>
                            </div>
    ...
    const editRow = async (id) => {
        ...
        return await fetch('<?= URL ?>panel/<?= $this->endpoint ?>/info/' + id, requestOptions)
            ...
            .then(result => {
                ...
                document.querySelector('fluent-text-field[name="tile_url"]').value = result.tile_url
