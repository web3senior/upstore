        ...
        <div class="ms-Grid-row">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-between align-items-center">
                                                ...
                                                <a href="<?= URL ?>panel/<?= $this->endpoint ?>/rate_delete?id=<?= $id ?>&company_id=<?= $_GET['company_id'] ?>"><?= ICON_DELETE ?></a>
                                            </div>
                                        </td>
                                    </tr>
                                <?php
            ...
            <div class="ms-Grid-col ms-sm12 ms-md12 ms-lg12" style="position:sticky;top:0">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="<?= URL ?>panel/<?= $this->endpoint ?>/rate_insert?company_id=<?= $_GET['company_id'] ?>" method="post" enctype="multipart/form-data" autocomplete="off">
...
<script type="text/javascript">
    ...
    const edit = async (id) => {
        ...
        return await fetch('<?= URL ?>panel/<?= $this->endpoint ?>/rate_info/' + id, requestOptions)
            .then(response => response.json())
            .then(result => {
                console.log(result)
                loading(false)
                document.querySelector('form').action = `<?= URL ?>panel/<?= $this->endpoint ?>/rate_update/${id}?company_id=<?= $_GET['company_id'] ?>`
